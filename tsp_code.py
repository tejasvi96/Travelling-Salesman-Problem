...
filename=r"city_coordinates.txt"
...
def configsetters(cfg):
    ...
    pop_size=len(data)
    weights=np.zeros((pop_size,pop_size),dtype=np.float)
    init_values=[]
    final_values=[]
    ans_values=[]
    for i in range(pop_size):
        for j in range(pop_size):
            weights[i][j]=(original_points[i][0] -original_points[j][0] )**2 + (original_points[i][1] -original_points[j][1] )**2
            weights[i][j]=weights[i][j]**0.5
    obj=GAalgo(tsp_len,pop_size,weights,iterations,elitism,crossover)
    ...
    joining_pts[0]=pts[-1]
    joining_pts[1]=pts[0]
    plt.title("Best Solution Tour using PMX Crossover")

    plt.plot(pts[0][0],pts[0][1],color='orange', marker='o',linestyle='dashed',label="Starting city")
    plt.plot(pts[1:,0],pts[1:,1],color='green', marker='o',linestyle='dashed')
    plt.plot(pts[-1][0],pts[-1][1],color='blue', marker='o',linestyle='dashed',label="Ending city")
    plt.plot(joining_pts[:,0],joining_pts[:,1],color='orange',linestyle='dashed')
    plt.legend()
    plt.show()
...
class GAalgo:
    ...
    def crossover(self,p,q,crossover_type):
        ...
        if crossover_type=="PMX":
            ...
            print("Wrong choice")
            print("Choose from 'PMX','Cycle','Order1','Order2','Position','Genetic','MPX','Alternation' ")
            exit()
        return c1,c2
    def run_algo(self):
